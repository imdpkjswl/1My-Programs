...
NODE insert_before(int given,int item,NODE first)
{
    NODE temp,CUR,PREV;
    ...
    /* Insert item before the given first node */
    if(given == first->Info)
    {
        temp = getNODE();
        temp->Info=item;
    ...
    /* Insert item after first node BUT before given node in the list */
    temp = getNODE();
    temp->Info=item;
    PREV->Link = temp;
    temp->Link=CUR;
...
void main()
{
    ...
    for(;;)
    {
        printf("1.Insert into List\n2.Display List\n3.Insert before given node in a List\n4.Exit\n");
        ...
        case 3:
            printf("Enter the element in which item to be insert before element :  ");
            scanf("%d",&given);
            printf("Enter the item to be inserted :  ");
            scanf("%d",&item);
            first=insert_before(given,item,first);
            break;
